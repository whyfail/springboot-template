...
- bcrypt 强度默认 12（`app.security.bcrypt-strength`，4..15）。启动时运行单次哈希基准（`PasswordHashBenchmark`），超 1.5s 记录警告。
...
- Redis 只存 `SHA-256(token)` 摘要（`app:session:{digest}`），原始 Token 不入 Redis/数据库/日志/审计/指标。
...
- 限流键对主体做 SHA-256 哈希（`app:login-rate:{kind}:{hash}:{bucket}`），Redis 中无原始 IP/用户名。
...
- CORS 显式白名单（`app.security.allowed-origins`），无 `*`、无凭据模式；仅 GET/POST/PUT/PATCH/DELETE/OPTIONS 与 `Authorization/Content-Type/X-Request-ID` 头。
