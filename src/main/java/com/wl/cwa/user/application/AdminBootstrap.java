 ...
 * {@code cwa.bootstrap.enabled=true} (local profile or an explicit operator decision) and the
 * password comes from the environment without any default. No migration ships an administrator.
 */
@Component
@ConditionalOnProperty(name = "cwa.bootstrap.enabled", havingValue = "true")
public class AdminBootstrap implements ApplicationRunner {
    ...
    public AdminBootstrap(
            CwaBootstrapProperties properties,
    ...
    public void run(ApplicationArguments args) {
        ...
        if (password == null || password.length() < 12) {
            throw new IllegalStateException(
                    "Bootstrap enabled but no administrator password provided via CWA_BOOTSTRAP_ADMIN_PASSWORD "
