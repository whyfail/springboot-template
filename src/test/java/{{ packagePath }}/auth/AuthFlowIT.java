...
import {{ package }}.support.ContainerTestSupport;
...
class AuthFlowIT extends ContainerTestSupport {
    ...
    private String login(String body, String remoteAddr) throws Exception {
        ...
        return {{ package }}.support.JsonText.extractString(response, "token");
    }
    ...
    void rememberFlagExtendsSessionTtlToThirtyDays() throws Exception {
        ...
        String digest = {{ package }}.shared.security.Hashes.sha256Hex(token);
    ...
    void invalidPasswordAndUnknownUserAreIndistinguishable() throws Exception {
        ...
        assertThat({{ package }}.support.JsonText.extractString(wrongPassword, "code"))
                .isEqualTo({{ package }}.support.JsonText.extractString(unknownUser, "code"));
        assertThat({{ package }}.support.JsonText.extractString(wrongPassword, "msg"))
                .isEqualTo({{ package }}.support.JsonText.extractString(unknownUser, "msg"));
        assertThat({{ package }}.support.JsonText.extractString(wrongPassword, "msg")).isEqualTo("账号或密码错误");
    }
    ...
    void expiredSessionIsRejectedAndCleanedUp() throws Exception {
        ...
        String digest = {{ package }}.shared.security.Hashes.sha256Hex("expired-token-value");
    ...
    void concurrentLoginsShareSessionLimitWithoutBreaking() throws Exception {
        ...
        assertThat(redis.opsForValue().get("app:session:" + {{ package }}.shared.security.Hashes.sha256Hex(first)))
                .isNull();
    }
