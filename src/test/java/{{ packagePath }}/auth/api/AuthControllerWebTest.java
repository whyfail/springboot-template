...
import {{ package }}.auth.application.LoginCommand;
...
class AuthControllerWebTest {
    ...
    void loginReturnsTopLevelToken() throws Exception {
        ...
        when(authService.login(any(), any()))
                .thenReturn(new {{ package }}.auth.api.dto.LoginResponse(
                        "opaque-token-value",
                        "Bearer",
                        expiresAt,
                        new {{ package }}.auth.api.dto.CurrentUserResponse(
                                UUID.fromString("00000000-0000-0000-0000-000000000042"),
                                "admin",
                                "Administrator",
                                null,
    ...
    void logoutWithoutTokenReturns401() throws Exception {
        org.mockito.Mockito.doThrow(new {{ package }}.shared.error.BusinessException(
                        {{ package }}.shared.error.ErrorCode.AUTH_REQUIRED))
                .when(authService)
                .logout(null, null);
    ...
    void meWithSessionPrincipalReturnsCurrentUserShape() throws Exception {
        when(authService.currentUser(any()))
                .thenReturn(new {{ package }}.auth.api.dto.CurrentUserResponse(
                        UUID.fromString("00000000-0000-0000-0000-000000000042"),
                        "admin",
                        "Administrator",
                        null,
