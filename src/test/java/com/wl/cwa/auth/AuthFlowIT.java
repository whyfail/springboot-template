...
import com.wl.cwa.audit.domain.AuditEventEntity;
import com.wl.cwa.audit.domain.AuditEventRepository;
...
class AuthFlowIT extends ContainerTestSupport {
    ...
    private String login(String body, String remoteAddr) throws Exception {
        ...
        return com.wl.cwa.support.JsonText.extractString(response, "token");
    }
    ...
    @Test
    void loginWithSpaAliasFieldsBehavesIdentically() throws Exception {
        createEnabledUser("aliased");

        mockMvc.perform(post("/api/v1/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"aliased\",\"password\":\"" + STRONG_PASSWORD + "\",\"checked\":true}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.token").isNotEmpty())
    ...
    void rememberFlagExtendsSessionTtlToThirtyDays() throws Exception {
        ...
        String digest = com.wl.cwa.shared.security.Hashes.sha256Hex(token);

        Long ttl = redis.getExpire("cwa:session:" + digest);
    ...
    void invalidPasswordAndUnknownUserAreIndistinguishable() throws Exception {
        ...
        assertThat(com.wl.cwa.support.JsonText.extractString(wrongPassword, "code"))
                .isEqualTo(com.wl.cwa.support.JsonText.extractString(unknownUser, "code"));
        assertThat(com.wl.cwa.support.JsonText.extractString(wrongPassword, "msg"))
                .isEqualTo(com.wl.cwa.support.JsonText.extractString(unknownUser, "msg"));
        assertThat(com.wl.cwa.support.JsonText.extractString(wrongPassword, "msg")).isEqualTo("账号或密码错误");
    }
    ...
    void rawTokenNeverAppearsInRedisOrIndex() throws Exception {
        ...
        assertThat(redis.keys("cwa:session:*")).contains("cwa:session:" + digest);
        for (String key : redis.keys("cwa:session:*")) {
            String value = redis.opsForValue().get(key);
            assertThat(value).isNotNull();
            assertThat(value.contains(token)).as("raw token leaked in %s", key).isFalse();
        }
        for (String key : redis.keys("cwa:user-sessions:*")) {
    ...
    void sixthLoginEvictsOldestSession() throws Exception {
        ...
        for (String token : tokens) {
            String digest = com.wl.cwa.shared.security.Hashes.sha256Hex(token);
            if (redis.opsForValue().get("cwa:session:" + digest) != null) {
                liveSessions++;
            }
        }
    ...
    void expiredSessionIsRejectedAndCleanedUp() throws Exception {
        ...
        String digest = com.wl.cwa.shared.security.Hashes.sha256Hex("expired-token-value");
        redis.opsForValue().set("cwa:session:" + digest, newJson(expired), Duration.ofMinutes(5));

        mockMvc.perform(get("/api/v1/me").header("Authorization", "Bearer expired-token-value"))
                .andExpect(status().isUnauthorized());

        assertThat(redis.opsForValue().get("cwa:session:" + digest)).isNull();
    }
    ...
    void concurrentLoginsShareSessionLimitWithoutBreaking() throws Exception {
        ...
        assertThat(redis.opsForValue().get("cwa:session:" + com.wl.cwa.shared.security.Hashes.sha256Hex(first)))
                .isNull();
    }
